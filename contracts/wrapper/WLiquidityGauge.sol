...
contract WLiquidityGauge is ERC1155('WLiquidityGauge'), ReentrancyGuard, IERC20Wrapper, Governable {
  ...
  constructor(ICurveRegistry _registry, IERC20 _mobi) public {
    __Governable__init();
    registry = _registry;
    mobi = _mobi;
  }

  /// @dev Encode pid, gid, mobiPerShare to a ERC1155 token id
  ...
  function encodeId(
    ...
    return (pid << 246) | (gid << 240) | mobiPerShare;
  }

  /// @dev Decode ERC1155 token id to pid, gid, mobiPerShare
  ...
  function registerGauge(uint pid, uint gid) external onlyGov {
    ...
    gauges[pid][gid] = GaugeInfo({impl: ILiquidityGauge(gauge), accMobiPerShare: 0});
  }
  ...
  function mint(
    ...
    impl.deposit(amount);
    uint id = encodeId(pid, gid, gauge.accMobiPerShare);
  ...
  function burn(uint id, uint amount) external nonReentrant returns (uint) {
    ...
    (uint pid, uint gid, uint stMobiPerShare) = decodeId(id);
